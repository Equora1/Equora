...
const CURRENT_PENDING_BOOTSTRAP_ATTEMPT_ID = 'mb0-remediation10-closure-manifest-001'
...
function validateAttemptCollections(evidence, allowPendingManifestAttempt) {
  ...
  for (const collection of ['baseline_attempts', 'candidate_attempts']) {
    ...
    for (const attempt of attempts) {
      ...
      if (typeof attempt.command !== 'string' || !attempt.command) fail(`Attempt enthält keinen exakten Befehl: ${attempt.attempt_id}`)
      if (typeof attempt.result !== 'string' || !attempt.result) fail(`Attempt enthält kein Ergebnis: ${attempt.attempt_id}`)
      if (attempt.started_at_utc !== null && !isIsoInstant(attempt.started_at_utc)) fail(`Attempt-Startzeit ist ungültig: ${attempt.attempt_id}`)
      if (attempt.ended_at_utc !== null && !isIsoInstant(attempt.ended_at_utc)) fail(`Attempt-Endzeit ist ungültig: ${attempt.attempt_id}`)
      if (isIsoInstant(attempt.started_at_utc) && isIsoInstant(attempt.ended_at_utc)
        && Date.parse(attempt.started_at_utc) > Date.parse(attempt.ended_at_utc)) {
...
function validateEvidence(reader, entries, manifestPath, allowPendingManifestAttempt) {
  ...
  if (evidence.phase !== 'MB0') fail('unerwartete Evidence-Phase')
  ...
  if (baselineCounts?.test_files !== EXPECTED_COUNTS.baseline_test_files
    || baselineCounts?.tests !== EXPECTED_COUNTS.baseline_tests
    || baselineCounts?.audit_all_vulnerabilities !== EXPECTED_COUNTS.audit_all_vulnerabilities
    || baselineCounts?.audit_production_vulnerabilities !== EXPECTED_COUNTS.audit_production_vulnerabilities
    ...
    || candidateCounts?.audit_all_vulnerabilities !== EXPECTED_COUNTS.audit_all_vulnerabilities
    || candidateCounts?.audit_production_vulnerabilities !== EXPECTED_COUNTS.audit_production_vulnerabilities) {
  ...
  validateAttemptCollections(evidence, allowPendingManifestAttempt)
