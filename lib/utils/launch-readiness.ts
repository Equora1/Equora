  ...
  const steps: LaunchStep[] = [
    ...
    createStep(
      hasReviewSeed
        ? {
            label: 'Review-Futter',
            value: dailyNotes.length > 0 ? `${dailyNotes.length} Review-Notizen als Basis` : `${trades.length} Trades als Basis`,
            detail: dailyNotes.length > 0
              ? 'Review-Notizen liegen schon bereit und geben dem Review echten Kontext.'
              : 'Auch ohne Review-Notizen ist genug Material da, damit Review und Statistik nicht im Vakuum arbeiten.',
            status: 'done',
            href: dailyNotes.length > 0 ? '/kalender' : '/review',
            cta: dailyNotes.length > 0 ? 'Review-Notizen ansehen' : 'Review öffnen',
          }
        : {
            label: 'Review-Futter',
            value: 'Noch dünn',
            detail: 'Für echte Review-Muster braucht Equora ein paar Trades oder eine Review-Notiz. Sonst bleibt die Analyse nur Theorie.',
            status: 'warning',
            href: '/review',
