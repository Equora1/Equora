...
import { buildTradesHref, shortenReviewText } from '@/lib/utils/review-helpers'
...
export function buildReviewLayerSnapshot(tradesCurrent: Trade[]): ReviewLayerSnapshot {
    ...
    highlights.push({
      ...
      detail: repeatYes > 0
        ...
        : repeatNo > 0
          ...
          : 'Noch kein klares Wiederholungsmuster in der Verhaltensprüfung.',
      tone,
  ...
  const headline = !totalTrades
    ...
    : coverage >= 70
      ? 'Die Verhaltensprüfung liefert schon belastbare Verhaltenstrends'
      : coverage >= 35
        ? 'Die Verhaltensprüfung zeigt erste Verhaltensmuster'
        : 'Die Verhaltensprüfung ist noch punktuell, aber schon nützlich'
  ...
  const summary = !totalTrades
    ...
    : reviewedTrades.length === 0
      ? 'Noch kein Trade im Zeitraum wurde in der Verhaltensprüfung nachgezogen. Genau dort werden aus Fakten echte Verhaltenssignale.'
      : `${reviewedTrades.length} von ${totalTrades} Trades tragen Review-Signale. ${ruleNo > 0 ? `${ruleNo} Trades fallen klar als Regelbruch auf.` : repeatYes > 0 ? `${repeatYes} Trades sehen nach replizierbarem Kernmaterial aus.` : 'Die Verhaltensprüfung zeigt erste Muster für Verhalten und P&L.'}`

  const checklist = [
    coverage < 70 ? 'Offene Trades im Edit-Bereich kurz mit Review-Signalen nachziehen' : null,
