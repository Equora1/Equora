...
async function ensureWorker() {
  if (!workerPromise) {
    emit({
      phase: 'preparing',
      progress: 0,
      status: 'Deutsch/Englisch-Sprachmodelle werden einmalig geladen... Das kann beim ersten Mal kurz dauern.',
    })
    ...
    workerPromise = (async () => {
      ...
      emit({
        phase: 'ready',
        progress: 100,
        status: 'OCR ist bereit. Die Deutsch/Englisch-Modelle sind geladen.',
      })
      return worker
    })().catch((error) => {
      workerPromise = null
