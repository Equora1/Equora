...
export function getTradeTrustMeta(trade: Trade): TradeTrustMeta {
  ...
  if (state === 'conflict') {
    return {
      state,
      label: 'Status passt nicht zur P&L',
      shortLabel: 'Konflikt',
      description: 'Status und P&L zeigen in unterschiedliche Richtungen. Bitte diesen Trade prüfen.',
      tone: 'red',
      reliable: false,
      needsAttention: true,
    }
  ...
  if (state === 'missing-pnl') {
    return {
      state,
      label: 'Geschlossen, aber ohne P&L',
      shortLabel: 'Ohne P&L',
  ...
  return {
    ...
    description:
      trade.pnlSource === 'derived'
        ? 'P&L wird aus Entry, Exit und Kosten berechnet und fließt belastbar in Statistiken ein.'
        : trade.pnlSource === 'manual' || trade.pnlSource === 'override'
          ? 'Der Trade ist belastbar genug für Auswertungen, basiert aber auf manuell gesetzter P&L.'
...
export function getTradeTrustChecklist(trade: Trade) {
  ...
  if (trade.netPnL === undefined || trade.netPnL === null) {
    items.push('P&L fehlt noch')
  } else if (trade.pnlSource === 'derived') {
    items.push('P&L wird aus Entry/Exit/Kosten berechnet')
  } else if (trade.pnlSource === 'manual') {
    items.push('P&L wurde manuell gesetzt')
  } else if (trade.pnlSource === 'override') {
    items.push('P&L überschreibt die Auto-Rechnung')
  }

  if (hasPnLConflict(trade)) {
    items.push('Status und P&L widersprechen sich')
  }
