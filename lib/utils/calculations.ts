...
export function derivePartialExitPnL({
  instrumentType,
  entry,
  bias,
  positionSize,
  ...
  const resolvedSize = parseTradingNumber(positionSize)
  const normalizedInstrument = normalizeInstrumentType(instrumentType)
  const instrumentPreset = getTradeInstrumentPreset(normalizedInstrument)
  const multiplier =
    normalizedInstrument === 'futures' || normalizedInstrument === 'forex' || normalizedInstrument === 'cfd'
      ? parseTradingNumber(pointValue) ?? instrumentPreset.defaultPointValue ?? null
      : 1
  ...
  const grossPnL = normalized.reduce((sum, leg) => {
    const legSize = (resolvedSize * leg.percent) / 100
    const priceMove = direction === 'short' ? entryValue - leg.price : leg.price - entryValue
  ...
  const weightedPriceMove = coveredPercent > 0
    ...
    : null
  const netPnL = grossPnL - costShare
  ...
  return {
    ...
    remainingSize,
    hasOpenRemainder: remainderPercent > 0.0001,
    costShare: Number.isFinite(costShare) ? costShare : 0,
    priceMove: weightedPriceMove !== null && Number.isFinite(weightedPriceMove) ? weightedPriceMove : null,
    multiplier,
  }
}
...
export function buildQuickPnLPreview({
  entry,
  exit,
  positionSize,
  pointValue,
  ...
  const pointValueNumber = parseTradingNumber(pointValue) ?? instrumentPreset.defaultPointValue
  const costValue = parseTradingNumber(totalCosts) ?? 0
  const missing: string[] = []

  if (direction === 'neutral') missing.push('Richtung')
  if (entryValue === null) missing.push('Entry')
  if (exitValue === null) missing.push('Exit')
  if (sizeValue === null || sizeValue <= 0) missing.push('Size')
  if ((normalizedInstrument === 'futures' || normalizedInstrument === 'forex' || normalizedInstrument === 'cfd') && (!pointValueNumber || pointValueNumber <= 0)) {
    missing.push('Punktwert')
  }
  ...
  const gross = deriveGrossPnL({
    ...
    entry: entryValue,
    exit: exitValue,
    positionSize: sizeValue,
    pointValue: pointValueNumber,
  })

  const netPnL = gross.grossPnL !== null ? gross.grossPnL - costValue : null

  return {
    netPnL: netPnL !== null && Number.isFinite(netPnL) ? netPnL : null,
...
export type TradeComputation = {
  ...
  multiplier: number | null
  rValue: number
  realizedRValue: number | null
  plannedRValue: number | null
  ...
  direction: 'long' | 'short' | 'neutral'
  rSource: 'manual' | 'realized' | 'realized_partial' | 'planned' | 'missing'
  instrumentType: TradeInstrumentType
  cryptoMarketType: TradeCryptoMarketType
  executionType: TradeExecutionType
  fundingDirection: TradeFundingDirection
  ...
  partialRealizedGrossPnL: number | null
}
...
export function computeTradeMetrics({
  ...
  const storedRValue = parseTradingNumber(rMultiple)
  ...
  const partialPlan = getPartialExitPlanInfo({ exit, partialExits })
  const partialRealized = derivePartialExitPnL({
  ...
  const effectiveNetPnL =
    pnlComputation.netPnL !== null
      ? pnlComputation.netPnL
      : partialPlan.count && partialRealized.netPnL !== null
        ? partialRealized.netPnL
        : null
  const effectiveAutoNetPnL =
    pnlComputation.autoNetPnL !== null
      ? pnlComputation.autoNetPnL
      : pnlComputation.source === 'missing' && partialPlan.count
        ? partialRealized.netPnL
        : null
  const effectiveGrossPnL =
    pnlComputation.grossPnL !== null
      ? pnlComputation.grossPnL
      : partialPlan.count
        ? partialRealized.grossPnL
        : null
  const effectivePriceMove =
    pnlComputation.priceMove !== null
      ? pnlComputation.priceMove
      : partialPlan.count
        ? partialRealized.priceMove
        : null
  const effectiveMultiplier =
    pnlComputation.multiplier !== null
      ? pnlComputation.multiplier
      : partialPlan.count
        ? partialRealized.multiplier
        : null
  const effectivePnlSource =
    pnlComputation.source === 'missing' && partialPlan.count && partialRealized.netPnL !== null
      ? 'derived'
      : pnlComputation.source
  const matchesPlannedStoredR =
    storedRValue !== null
    && plannedR !== null
    && Math.abs(storedRValue - plannedR) < 0.0001
  const legacyStoredR = matchesPlannedStoredR ? null : storedRValue
  ...
  const realizedR =
    effectiveNetPnL !== null && stopRiskAmount !== null && stopRiskAmount > 0
      ? effectiveNetPnL / stopRiskAmount
      : null
  const derivedR = realizedR ?? plannedR
  const manualR = derivedR === null ? legacyStoredR : null
  const rValue = derivedR ?? manualR ?? 0
  const rSource =
    realizedR !== null
      ...
      : plannedR !== null
        ...
        : manualR !== null
          ? 'manual'
          : 'missing'
  const pnlBasedRiskAmount = effectiveNetPnL !== null && rValue !== 0 ? Math.abs(effectiveNetPnL / rValue) : null
  ...
  return {
    ...
    autoNetPnL: effectiveAutoNetPnL,
    grossPnL: effectiveGrossPnL,
    pnlSource: effectivePnlSource,
    ...
    priceMove: effectivePriceMove,
    multiplier: effectiveMultiplier,
    rValue,
    realizedRValue: realizedR,
    plannedRValue: plannedR,
    ...
    partialRealizedGrossPnL: partialPlan.count ? partialRealized.grossPnL : null,
  }
}
