...
export function CalendarDayCard({
  ...
  return (
    <FuturisticCard
      glow={glow}
      className={`min-h-[136px] p-0 ${!isCurrentMonth ? 'opacity-50' : ''} ${todayRing} ${selectedRing}`}
    >
      <button
        type="button"
        onClick={onSelect}
      ...
      >
        <div className="flex items-start justify-between gap-2">
          <span className={`text-sm font-semibold ${isToday && !summary ? 'text-orange-100' : 'text-white'}`}>{day}</span>
          {isToday && !summary ? (
            <span className="rounded-full border border-orange-400/25 bg-orange-400/10 px-2 py-0.5 text-[10px] text-orange-100">Heute</span>
          ) : summary ? (
            <span
