...
export function DailyNoteFlowCard({
  ...
  return (
    <FuturisticCard glow={glow} className={compact ? 'p-5' : 'p-6'}>
      <SectionHeader
        eyebrow="Daily Note"
        title={summary.existingNote ? 'Daily Note aktualisieren' : 'Daily Note schreiben'}
        copy={summary.copy}
      ...
      <div className={`grid gap-6 ${compact ? 'xl:grid-cols-[1.1fr_0.9fr]' : 'xl:grid-cols-[1.12fr_0.88fr]'}`}>
        <div className="space-y-4 rounded-[30px] border border-orange-400/15 bg-black/35 p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              <p className="text-xs uppercase tracking-[0.24em] text-orange-200/75">Heute</p>
          ...
          <div className={`grid gap-4 ${compact ? 'lg:grid-cols-[1.15fr_0.85fr]' : 'lg:grid-cols-[1.1fr_0.9fr]'}`}>
            <div className="rounded-[26px] border border-orange-400/15 bg-orange-400/5 p-5">
              <p className="text-[10px] uppercase tracking-[0.24em] text-orange-100/70">Kurzstand</p>
            ...
            <div className="rounded-[26px] border border-white/10 bg-white/[0.03] p-5">
              <p className="text-[10px] uppercase tracking-[0.24em] text-white/35">Auffällig</p>
          ...
          <div className={`grid gap-4 ${compact ? 'lg:grid-cols-[0.95fr_1.05fr]' : 'lg:grid-cols-2'}`}>
            <div className="rounded-[26px] border border-white/10 bg-white/[0.03] p-5">
              <p className="text-[10px] uppercase tracking-[0.24em] text-white/35">Drei Fragen</p>
            ...
            {summary.dayTrades.length ? (
              <div className="rounded-[26px] border border-white/10 bg-black/20 p-5">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-[10px] uppercase tracking-[0.24em] text-white/35">Trades</p>
                    <p className="mt-2 text-sm text-white/58">Die wichtigsten Trades des Tages bleiben schnell erreichbar.</p>
                  </div>
              ...
              <div className="rounded-[26px] border border-white/10 bg-black/20 p-5 text-sm text-white/55">
                Für dieses Datum liegt noch kein Trades vor.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4 rounded-[30px] border border-white/10 bg-white/[0.03] p-5">
          <div className="rounded-[26px] border border-orange-400/15 bg-orange-400/5 p-4">
            <p className="text-[10px] uppercase tracking-[0.24em] text-orange-100/70">Morgen</p>
            <p className="mt-2 text-sm leading-6 text-white/68">
              Ein Satz reicht: Was soll morgen sauberer werden?
            </p>
          </div>

          <div className="rounded-[26px] border border-white/10 bg-black/20 p-4">
            <p className="text-[10px] uppercase tracking-[0.24em] text-white/35">Fokus für morgen</p>
            <p className="mt-2 text-sm leading-6 text-white/68">Ein klarer Fokus reicht.</p>
          ...
          <label className="block">
            <span className="text-[10px] uppercase tracking-[0.24em] text-white/35">Fokus</span>
            <input
              value={focus}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className={`text-sm ${status ? (status.ok ? 'text-emerald-300' : 'text-orange-200') : 'text-white/45'}`}>
              {status?.text ?? (isMock ? 'Im Demo-Modus ist die Daily Note sichtbar, aber nicht schreibbar.' : 'Daily Notes werden user-gebunden gespeichert.')}
            </p>
